...
export function UserManagement() {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-14 text-center text-sm text-slate-500">{"\u7528\u6237\u6570\u636e\u52a0\u8f7d\u4e2d..."}</div> : <div className="grid gap-4">{rows.map((user) => <section key={user.id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><h3 className="text-lg font-semibold text-slate-900">{user.displayName}</h3><p className="mt-1 text-sm text-slate-500">{`${user.username} · ${user.phone || "\u672a\u7ed1\u5b9a\u624b\u673a"}`}</p></div><label className="inline-flex items-center gap-3 text-sm text-slate-600"><span>{"\u89d2\u8272"}</span><select value={user.role} onChange={(event) => void updateUser(user, event.target.value as UserRecord["role"], user.effectivePermissions)} className="rounded-xl border border-slate-200 px-3 py-2 outline-none focus:border-brand"><option value="ADMIN">{"\u7ba1\u7406\u5458"}</option><option value="MANAGER">{"\u6210\u5458"}</option><option value="CLERK">{"\u6210\u5458"}</option></select></label></div><div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{permissionKeys.map((key) => <label key={key} className="flex items-center justify-between rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700"><span>{permissionLabels[key]}</span><input type="checkbox" checked={user.effectivePermissions[key]} disabled={user.role === "ADMIN" || savingId === user.id} onChange={(event) => void updateUser(user, user.role, { ...user.effectivePermissions, [key]: event.target.checked })} className="h-4 w-4 rounded border-slate-300 text-brand focus:ring-brand" /></label>)}</div></section>)}</div>}
    </div>
  );
}
